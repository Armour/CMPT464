...
float LoopVertexMaskBeta(int k) {
    float tmp = 0.375 + 0.25 * cos(2 * 3.1415926 / k);
    float beta = (0.625 - tmp * tmp) / k;
    return beta;
}
...
float ButterflyVertexMaskSj(int k, int j) {
    float tmp = 2 * j * 3.1415926 / k;
    float Sj = (0.25 + cos(tmp) + 0.5 * cos(2 * tmp)) / k;
    return Sj;
}
...
void LoopSubdivision(int level) {
    ...
    for (int i = 0; i < level; i++) {
        ...
        for (auto vertex : old_mesh_vertex) {
            ...
            float beta = subdivision::LoopVertexMaskBeta(k);
            x = (1 - k * beta) * vertex->x;
            y = (1 - k * beta) * vertex->y;
            z = (1 - k * beta) * vertex->z;
            edge = e0;
        ...
        for (auto edge : old_mesh_edges) {
            ...
            if (edge_midpoint_index_map.find(make_pair(old_vertex_index_map[ed], old_vertex_index_map[st])) != edge_midpoint_index_map.end()) {
                ...
                tmpVec.push_back(z);
                vertex_input.push_back(tmpVec);
                edge_midpoint_index_map[make_pair(old_vertex_index_map[st], old_vertex_index_map[ed])] = old_mesh_vertex.size() + edge_midpoint_count++;
            }
        }
            ...
            if (edge_midpoint_index_map.find(make_pair(old_vertex_index_map[ed], old_vertex_index_map[st]))
                ...
                if (k1 == 6 && k2 == 6) {               // Regular cases
                    ...
                    if (k1 > 4) {
                        ...
                        p = e0;
                        do {
                            float Sj = ButterflyVertexMaskSj(k1, j);
                            if (p->end == st) {
                                x += Sj * p->start->x;
                                y += Sj * p->start->y;
                                z += Sj * p->start->z;
                ...
                } else if (k2 != 6) {               // Extraordinary cases
                    if (k2 > 4) {
                        int j = 0;
                        float q = 1.0;
                        ...
                        p = e0;
                        do {
                            float Sj = ButterflyVertexMaskSj(k2, j);
                            if (p->start == ed) {
                                x += Sj * p->end->x;
                                y += Sj * p->end->y;
                                z += Sj * p->end->z;
